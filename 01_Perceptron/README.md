...
퍼셉트론의 실제 정답 `Y`와 예측값 `y_pred` 사이에 얼마나 차이가 있는지 확인합니다.

이번 실습에서는 대표적인 오차 측정 방법인 **MSE**와 **MAE**를 계산합니다.
...
**평균 제곱 오차**로, 실제값과 예측값의 차이를 **제곱한 후 평균**을 계산합니다.
...
따라서 잘못 분류된 데이터가 많을수록 MSE가 커집니다.

---

### 11.2 MAE (Mean Absolute Error)

**평균 절대 오차**로, 실제값과 예측값의 차이에 **절댓값을 적용한 후 평균**을 계산합니다.
...
| 지표 | 계산 방법 | 특징 |
|---|---|---|
| **MSE** | 오차를 제곱하여 평균 | 큰 오차를 더 크게 반영 |
| **MAE** | 오차의 절댓값을 평균 | 오차 크기를 직관적으로 표현 |
